...
import { isDynamicImportError, useVersionReload } from "../src/browser/version.js";
...
describe("useVersionReload", () => {
  ...
  it("reloads once per version and writes the session marker", async () => {
    ...
    expect(window.sessionStorage.getItem("observability-reloaded-for")).toBe(NEW);
  });
  ...
  it("never prompts again for a version the browser already reloaded for", async () => {
    ...
    window.sessionStorage.setItem("observability-reloaded-for", NEW);
    mockVersion(NEW);
    const { result } = renderHook(() => useVersionReload({ currentVersion: CURRENT }));
  ...
  it("clears the marker once bundle and server versions match", async () => {
    window.sessionStorage.setItem("observability-reloaded-for", CURRENT);
    mockVersion(CURRENT);
    const { result } = renderHook(() => useVersionReload({ currentVersion: CURRENT }));
    ...
    expect(window.sessionStorage.getItem("observability-reloaded-for")).toBeNull();
  });
