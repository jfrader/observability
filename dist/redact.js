...
const SENSITIVE_QUERY_VALUE_PATTERN = /([?&](?:access_token|api_key|apikey|code|id_token|identity|jwt|password|refresh_token|secret|state|token)\s*=)[^&#\s]*/gi;
/** Long opaque tokens (e.g. team join links, bearer-ish strings). */
const STANDALONE_LONG_TOKEN = /(^|[^A-Za-z0-9_-])[A-Za-z0-9_-]{43}(?=$|[^A-Za-z0-9_-])/g;
/**
 ...
 */
export function redactLongTokens(value, tokenRegex = STANDALONE_LONG_TOKEN) {
 ...
 */
export function redactBreadcrumbUrls(breadcrumb, options) {
